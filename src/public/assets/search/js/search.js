    ...
    function Search(entryList, inputJqs) {
        ...
        for (var i = 0; i < this.inputJqs.length; i++) {
            ...
            inputJq.keyup(function (e) {
                ...
                ;
                if (e.keyCode == 13) {
                    if (that.lastSearch != this.value) {
                        clearTimeout(that.statTimeout);
                        that.requestEntries(this.value, true);
                    }
                }
                if (e.keyCode === 27)
                    return;
...
var EntryList = (function () {
    ...
    EntryList.prototype.buildUrl = function (searchStr, writeStat) {
        if (writeStat) {
            return this.elemJq.data("url") + "&ss=" + encodeURI(searchStr) + "&stat=1";
        }
        return this.elemJq.data("url") + "&ss=" + encodeURI(searchStr);
    };
